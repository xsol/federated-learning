...
def main():
    ...
    if args.save_model:
        torch.save(model.state_dict(), f"{base_dir}/mnist_full_20dim_{args.epochs}epo.pt")
        
    from torch.utils.tensorboard import SummaryWriter
    # https://pytorch.org/tutorials/intermediate/tensorboard_tutorial.html
    # default `log_dir` is "runs" - we'll be more specific here
    writer = SummaryWriter(f'{base_dir}/runs/mnist_full_{args.epochs}epo')
    dataiter = iter(train_loader)
    images, labels = next(dataiter)
    writer.add_graph(Net20(), images)
    writer.close()
    ...
    images, targets = select_n_random(dataset1.data, dataset1.targets)
    model.eval()
    with torch.no_grad():
            for data, target in test_loader:
                print(data.size())
